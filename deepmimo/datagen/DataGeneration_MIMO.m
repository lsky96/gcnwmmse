...
for i_bs = 1:K
    ...
    for i_user = 1: I
        % Training
        H = zeros(trainingdata_setsize, 2, M);
        for i_sample = 1:trainingdata_setsize
            H(i_sample, 1, :) = DeepMIMO_dataset{i_bs}.user{training.user_idx(i_sample, i_user)}.channel.';
            if mod(training.user_idx(i_sample, i_user), width) == 0
                offset = -1; % end of row, then take position before to stay in row
            else
                offset = +1;
            end
            H(i_sample, 2, :) = DeepMIMO_dataset{i_bs}.user{training.user_idx(i_sample, i_user) + offset}.channel.';
        end
        training.channels{i_bs}{i_user} = H;
        
        % Test
        H = zeros(testdata_setsize, 2, M);
        for i_sample = 1:testdata_setsize
            H(i_sample, 1, :) = DeepMIMO_dataset{i_bs}.user{test.user_idx(i_sample, i_user)}.channel.';
            if mod(test.user_idx(i_sample, i_user), width) == 0
                offset = -1; % end of row, then take position before to stay in row
            else
                offset = +1;
            end
            H(i_sample, 2, :) = DeepMIMO_dataset{i_bs}.user{test.user_idx(i_sample, i_user) + offset}.channel.';
        end
